-- SECURITY FIX: Update all SECURITY DEFINER functions to include proper search_path
-- without dropping them first to avoid dependency issues
...
-- SECURITY FIX: Consolidate duplicate update_user_credits functions
-- Remove only the decimal version, keep integer version with proper security
...
-- SECURITY FIX: Secure user_referrals table - restrict INSERT to authenticated users only
...
-- SECURITY FIX: Restrict app_settings access - only admins can view sensitive settings
