...
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_voice_response_user_thought
ON public.voice_responses (user_id, thought_id)
WHERE user_id IS NOT NULL;

-- For anonymous users (user_session is not null and user_id is null)
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_voice_response_session_thought
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_vote_user_voice
ON public.user_votes (user_id, voice_response_id)
WHERE user_id IS NOT NULL;

-- For anonymous users
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_vote_session_voice
...
-- =====================================================
CREATE INDEX IF NOT EXISTS idx_voice_responses_thought_id 
ON public.voice_responses (thought_id);

CREATE INDEX IF NOT EXISTS idx_voice_responses_user_id 
ON public.voice_responses (user_id) WHERE user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_user_votes_voice_response_id 
ON public.user_votes (voice_response_id);

CREATE INDEX IF NOT EXISTS idx_user_votes_user_id 
